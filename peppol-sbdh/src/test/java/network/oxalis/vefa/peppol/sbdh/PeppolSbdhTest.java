...
public class PeppolSbdhTest {
    ...
    @Test
    public void version201() throws IOException, SbdhException {
        try (InputStream inputStream = getClass().getResourceAsStream("/peppol-sbdh-2.01.xml")) {
            Header header = SbdhReader.read(inputStream);
            ...
            Assert.assertEquals(
                    ...
                    C1CountryIdentifier.of("IN"));
            Assert.assertEquals(
                    header.getMlsToIdentifier(),
                    MlsToIdentifier.of(
                            "0242:000723",
                            MlsToIdentifier.DEFAULT_SCHEME
                    ));
            Assert.assertEquals(
                    header.getMlsTypeIdentifier(),
